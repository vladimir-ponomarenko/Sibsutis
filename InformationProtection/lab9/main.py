...
def generate_elgamal_keys(bits: int = 32) -> Tuple[int, int, int, int]:
    """
    Генерирует ключи для подписи Эль-Гамаля.
...
def load_key(filepath: str) -> Tuple[int, int, int]:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Электронная подпись Эль-Гамаля")
    ...
    try:
        if args.command == "generate":
            print(f"Генерация {args.bits}-битных ключей Эль-Гамаля...")
            p, g, x, y = generate_elgamal_keys(args.bits)
            ...
            print(f"Открытый ключ (p, g, y) сохранен в: {args.pub}")
            print(f"Закрытый ключ (p, g, x) сохранен в: {args.priv}")
