...
1. **Ослепление**: Избиратель ослепляет свой выбор случайным фактором
2. **Подписание**: Сервер подписывает ослепленное сообщение
...
- Генерация случайных факторов ослепления
- Операции с обратными элементами
- Проверка подписей
...
- Сервер генерирует пару ключей (x, y), где y = g^x mod p

**Математические основы**:
- Все операции выполняются по модулю p
- Используется свойство: g^(p-1) ≡ 1 (mod p)
- Сервер знает закрытый ключ x, все знают открытый ключ y

### 2. Процесс голосования
**Выбор избирателя**:
- Избиратель выбирает вариант голосования (например, "Да", "Нет", "Воздержался")
- Выбор представляется строкой или числом

**Генерация хеша**:
- Выбор хешируется с помощью SHA-256: H = SHA256(choice)
- Хеш преобразуется в число: m = int(H, 16) mod p

**Ослепление сообщения**:
- Выбирается случайный фактор ослепления r ∈ [1, p-1]
- Сообщение ослепляется: m' = m * y^r mod p
- Ослепленное сообщение отправляется на сервер
